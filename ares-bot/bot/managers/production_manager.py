...
class ProductionManager(Manager):
    def __init__(
        ...
        # O363-①(o362b g3 实证):新矿落地即配塔的 per-base 节流台账
        # ((round(x),round(y)) → 上次钉点时刻;原全局节流只服务首矿)。
        self._o363_base_def_last: dict = {}
        # O363-②a(o362b g2/o362a g1 实证):钉点等钱的跨 pop 累计台账
        # (sid → 首次挂出时刻)—— tracker 条目被 O307 撤派工/O110 自救/
        # O118-② 快回收 pop 后 TIME_ORDER_COMMENCED 归零,60s 保险丝
        # 永不积累;条目清空(开工/释放)才销账。
        self._o363_pin_since: dict = {}
        # O363-②b(o362b g2 实证):O307 撤派工后的 60s 防夺回封锁
        # (sid → 封锁截止时刻,期内 _dispatch_structure 拒同型重钉)。
        self._o363_repin_block_until: dict = {}
        # O363-④d:O218「气烂银行追加星门」事件 30s 节流时刻(rc 振荡
        # 期 rc-change 簿记每帧刷屏实证)。
        self._o218_log_ts: float = 0.0
        # O364-①(o363a g3 直接死因):Nexus 资金窗独占旗标 —— 首扩
        # 等钱 >60s 时 luxury 钉点族(SG2+/FB/风暴/航母/Robo/Twilight)
        ...
        # O369-①b:计数改累计口径(不再要求连续),清零只在解除处。
        self._fb_nomoney_streak: int = 0
        self._fb_bankrupt: bool = False
        # O369-①(o368 双 lane 尸检):FB fund-first latch 簿记 ——
        # ①c stall 健康监控的 10s 滑窗矿采样(时刻/矿量)与零积累
        # 起点(持续 30s 零积累临时解除),临时解除截止(60s 后重
        # 评估);__init__ 初始化。
        self._fb_latch_m_sample_t: float = 0.0
        ...
        # O369-②a:销账口径改 sg_idle_reset_needed(只在产舰队单位
        # 才销,在产虚空/先知填线保留计时);FB 落成不再销账(②b
        # post-FB 矿穷填线读同一计时器)。
    ...
    async def update(self, iteration: int) -> None:
        ...
        if _o381_reason != self._o381_nexus_fund_reason:
            if _o381_reason is not None:
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
                    "msg": (
                        "O382:首扩220s硬基金启动(270s前Nexus必须落地)"
        ...
        # O382-①(o381a g1 局终尸检):基金虽在 250.0s 启动，ares 的
        # BuildOrderRunner 却在 ProductionManager 之外继续执行到 312s，
        # 依次花钱下叉/探机/水晶/BY/SG，Nexus 被拖到 368.2s。基金
        # 启动即截断剩余 opener；set_build_completed 同时释放持久建造工，
        # 未完成的核心科技待 Nexus 成交后由常态生产层补齐。
        _o382_bor = getattr(self.ai, "build_order_runner", None)
        if (
            _o382_bor is not None
            and nexus_fund_should_cut_build_runner(
                self._o381_nexus_fund_active,
                _o382_bor.build_completed,
            )
        ):
            _o382_step = getattr(_o382_bor, "build_step", -1)
            _o382_bor.set_build_completed()
            self.ai._events.append({
                "t": round(self.ai.time, 1),
                "msg": f"O382:基地基金截断开局runner(停在step#{_o382_step})",
            })
        ...
        if not (
            self._o381_nexus_fund_active
            or self._o381_healthy_expand_active
        ) and expand_holding_should_abort(
            ...
            if not _o336_keep:
                ...
                for _tag, _info in list(_tracker.items()):
                    if _info[TRACKER_ID] == UnitID.NEXUS:
                        ...
                        _tracker.pop(_tag)
                # O363-②b(o362b g2 实证):撤派工后 60s 防夺回封锁 ——
                # g2 撤派工 4 次(255/317/393/460s),每次 30s 冷却一过
                # 被同一钉点夺回,开矿钉点等钱 290s、二矿拖到 498s;
                # 封锁期内 _dispatch_structure 拒 NEXUS 重钉(pin_fuse_cd),
                # 资金/科技链真正解锁一轮。
                self._o363_repin_block_until[UnitID.NEXUS] = self.ai.time + 60.0
                # O363-②a:撤派工同步销累计台账(下一轮重钉重计等钱)
                self._o363_pin_since.pop(UnitID.NEXUS, None)
                self._expand_holding_since = None
                self._expand_abort_until = self.ai.time + 30.0  # O309-③:45→30
                _expand_holding = False
            else:
                # O364-①(o363a g3 直接死因):方向反转 —— 旧「解锁科技链
                # 30s」实证是科技消费(SG 261s/FB 325s/3 风暴/Robo/
                # Twilight ≈2000+ 矿)把 Nexus 的 400 矿窗永久挤掉
                # (首扩 470s 到死没落成,单矿 31 农,752s 被 54-supply
                # 波破主基)。改为:派工与 holding 都保留(科技链持续
                # 让位),luxury 钉点族 hold 45s(_o364_nexus_fund_hold
                # 闸 SG2+/FB/风暴/航母/Robo 各钉点),Nexus 钉点独占
                # 资金窗;成交(开工)/45s 超时/threat 豁免放行,防死锁。
                self._expand_holding_since = None
            ...
            self.ai._events.append({
                "t": round(self.ai.time, 1),
                "msg": (
                    "O364:首扩等钱>60s,luxury钉点hold45s,Nexus独占资金窗"
                    if _o336_keep
        ...
        # 覆盖开工快/落成慢/塔被拆补建全部形态。
        # O363-①(o362a/o362b 尸检):新矿落地即配塔,三处修补 ——
        # ① 旧循环钉完第一个分矿即 break + 30s 节流是全局单簿记,
        #    3/4 矿整局没人管(o362b g3 四矿 522s target=0 被连抄三次,
        #    累计撤 69 农人次);改 per-base 节流(_o363_base_def_last)
        #    并遍历全部非主基基地;
        # ② 电池同钉(cyber 就绪且 12 格内 0 电池)—— 单塔无奶 = 一次性
        #    防御(O256-③ 双电池奶塔存活×3 实证);
        # ③ 判据抽纯函数 new_base_defense_pins;通道仍是 fb_fund 同款
        #    critical 钉点(无视资金守卫,threat 时天然优先)。
        # O364-②(o363b 三局一致 + o363a g1 实证):配塔提前到「Nexus
        # 开工」并发 —— ① lane 闸 timing → timing+rush:旧闸下 Rush
        # lane 本守卫从不运行,新矿塔靠 F2 余钱,落成时才开始攒 350
        # 矿(2 塔+1 电池),落成后 +68-71s 塔才在途,验收线 60s
        # 稳定差一口气;townhalls 含在建(ares main.py:691),本循环
        # 的触发帧即「开工」,开工即发钉点,落成时塔已在途/已落成;
        # ② 在建 Nexus 的首座塔豁免 FB 基金窗(new_base_cannon_
        ...
        if expansion_defense_guard_active(self._opp_race, self._ai_build):
            for _exp_th in self.ai.townhalls:
                ...
                if (
                    self.ai.time - self._o363_base_def_last.get(_bk, -9999.0)
                    <= 30.0
                ):
                    continue
                ...
                _pin_cn, _pin_batt = new_base_defense_pins(
                    ...
                    cannons_near=_cn_near,
                    cannons_in_flight=_cn_if,
                    batteries_near=sum(
                        1
                        for s in self.ai.structures.ready
                    ...
                    batteries_in_flight=self._in_flight_near(
                ...
                # _surv_exempt 即保命塔口径)。
                # O369-⑤:塔投资冻结期分矿守卫同样停钉(首座保命塔
                # 豁免;o368b g2 的 25 塔/6 舰队反面教材,钱让给舰队)。
                if (
                    _pin_cn
                    and (not self._fb_bankrupt or _surv_exempt)
                    and (not self._o369_cannon_freeze or _surv_exempt)
                ):
        ...
        # O369-①(o368 双 lane 尸检):FB fund-first latch —— 触发口径
        # 从 O368-①b 的「no_money 连击 ≥3」改 latch 常态生效(o368a
        # 三局基金窗 0 开、O110 连钉 12 次落成 0 次实证:矿 5-756
        # 反复被 Nexus/塔/电池/虚空抢走):SG 就绪且 FB 无实体且非
        # threat 且(矿 <300 或气 <200)即开暂停(升级/SG2/分矿塔/
        # 电池四闸沿用 _fb_bankrupt,保命塔豁免;O369 追加探机(农
        # ≥28)/三矿+让位,见 _build_probes/_want_dynamic_expand)。
        # 兜底触发:no_money **累计** ≥3(不再要求连续,o368a g2
        # rush 常亮把连击打断后再也攒不到 3)或 FB 缺失 >120s(仅
        # SG 就绪+truly_missing 口径,opener 期不误触)。解除去掉
        # rush 常亮屏蔽:只 threat_active 临时解除,rush 期也保
        # latch。①c stall 防死锁:latch 期矿净积累 ≤0 持续 30s
        ...
        if self._fb_bankrupt:
            if self._fb_latch_m_sample_t == 0.0:
                self._fb_latch_m_sample_t = self.ai.time
                self._fb_latch_m_sample_v = self.ai.minerals
            elif self.ai.time - self._fb_latch_m_sample_t >= 10.0:
                if self.ai.minerals - self._fb_latch_m_sample_v <= 0.0:
                    if self._fb_latch_stall_since is None:
                        self._fb_latch_stall_since = self.ai.time
                else:
                    self._fb_latch_stall_since = None
                self._fb_latch_m_sample_t = self.ai.time
                self._fb_latch_m_sample_v = self.ai.minerals
            if fb_latch_stalled(self._fb_latch_stall_since, self.ai.time):
                self._fb_bankrupt = False
                self._fb_latch_release_until = self.ai.time + 60.0
                self._fb_latch_stall_since = None
                self._fb_latch_m_sample_t = 0.0
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
                    "msg": (
                        f"O369:FB latch30s矿净积累≤0,临时解除60s"
                        f"(矿{self.ai.minerals:.0f})"
                    ),
                })
        # O368-①b(o367 双 lane 尸检):FB 破产分支解除评估 —— 触发
        # 在 O110 自救与 O369-① latch 常态判据;FB 实体出现(钉下去
        # 了)或 threat 激活即解除,连击计数同步清零重新积累。窄域
        # 暂停(O360 先例),非全局资金冻结。
        # O369-①b:解除只传 threat_active(去掉 rush 常亮屏蔽 ——
        # o368a g2 破产分支 526s 解除后 rush 常亮,latch 再也保不住,
        # FB 到死没落);stall 台账同步销。
        if fb_bankrupt_cleared(_fb_entities_now, self._threat_active):
            if self._fb_bankrupt:
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
                    "msg": (
        ...
        _probe_yield = transition_probe_yield(
            ...
            # O369-①a(o368a g2 实证):FB latch 期探机同口径让位
            # (农≥28 才停,小农局探机是收入本身)—— g2 矿 5-756
            # 反复被探机/塔/电池/虚空抢走,FB 连钉 12 次落成 0 次。
            self._fb_bankrupt, self.ai.supply_workers
        ...
        # supply vs 此前 8-12),基地照丢且经济更穷。农民的账不能按
        # 「饱和溢出」算 —— 首波战损 9-12 后,超产的 4-8 农恰是重建火种。
        ...
        if (
            ...
            # O364-①:Nexus 资金窗独占期第 2+ 星门 hold(O218 追加
            # SG 前提含首舰已出 → 恒为 2+ 座,属 hold 清单)
            and not (
            ...
            if self._opp_race == "zerg" and self._ai_build == "timing":
                ...
                # 不节流派工,O357-④ 规约)。
                if _rc != getattr(self, "_o218_last_rc", None) and event_throttle_ok(
                    self.ai.time, self._o218_log_ts
                ):
                    self._o218_last_rc = _rc
                    self._o218_log_ts = self.ai.time
                    self.ai._events.append({
                        "t": round(self.ai.time, 1),
                        "msg": (
                ...
                await self._build_core_structure(UnitID.STARGATE)
                # O364-⑤c(o363b g1 实证):非 ZT 分支簿记原本零节流 ——
                # rush lane 走本分支,条件持续成立时每帧 append
                # (674-729s 刷上百条);与 ZT 分支同 30s 节流
                # (只节流言、不节流建造,O357-④ 规约)。
                if event_throttle_ok(self.ai.time, self._o218_log_ts):
                    self._o218_log_ts = self.ai.time
                    self.ai._events.append({
                        "t": round(self.ai.time, 1),
                        "msg": (
        ...
        # 等死。FB 落成后:就绪 SG 全忙(产线满载,加产能不浪费)
        # 且舰队(含在产)<8 且 SG 总数 <3 → 直接 critical 钉
        # SG2/SG3;矿 <150 不钉(O367-⑤b 矿门兼容:穷局钉点
        # no_money 空转,o366b 三次实证)。与 ① latch 不冲突:
        # latch 只活在 FB 未落时,本钉要求 FB 已落。
        _sg_ready_o369 = [
        ...
        if (
            ...
            and sg_gap_pin_needed(
                ...
                ready_sg_all_busy=all(
                    not s.is_idle for s in _sg_ready_o369
                ),
            ...
            if _rc == "dispatched":
                ...
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
                    "msg": (
                        f"O369:舰队缺口硬钉星门(SG={_sg_total_o218},"
                        f"舰队={_fleet_total_now})"
                    ),
                })
            elif event_throttle_ok(self.ai.time, self._o369_sg_gap_ts):
                # 失败 30s 节流(只节流言不节流钉点,O357-④ 规约)
                self._o369_sg_gap_ts = self.ai.time
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
        ...
        if (
            _fleet_starved
            and not self._transition_active
            # O364-①:Nexus 资金窗独占期 FB 钉点 hold(300/200 是
            # Nexus 400 矿窗的同台竞争者;threat 豁免在判据内)
            and not (
                self._o364_nexus_fund_hold()
        ...
        if (
            ...
            # O364-①:Nexus 资金窗独占期航母(350 矿)hold
            ...
            for _sg in self.manager_mediator.get_own_structures_dict[
                ...
                if _sg.is_ready and _sg.is_idle:
                    ...
                    break
        # O364-④(o363b g2 实证):航母硬转化 —— 矿烂银行局航母转化
        # 缺失(农 72/矿 955 烂银行,航母只有 1 艘;O239 的气 ≥400 门
        # 在矿烂气平局不 trigger;Rush lane 航母首产 755-900s vs
        # Power 胜局 590-739s;暴风为主的舰队遇腐化 12-16 零还手)。
        # FB 就绪 + 矿 >600 + 航母(含在产)<2 → 空闲就绪星门直接
        ...
        if (
            ...
            and carrier_hard_convert_ok(
                ...
                can_afford=self.ai.can_afford(UnitID.CARRIER),
            )
            and not self._ms_window
            # O364-①:Nexus 资金窗独占期同 hold(与 O239 同口径)
            and not self._o364_nexus_fund_hold()
        ):
            for _sg in self.manager_mediator.get_own_structures_dict[
                UnitID.STARGATE
                ...
                if _sg.is_ready and _sg.is_idle:
                    _sg.train(UnitID.CARRIER)
                    if event_throttle_ok(self.ai.time, self._o364_carrier_log_ts):
                        self._o364_carrier_log_ts = self.ai.time
                        self.ai._events.append({
                            "t": round(self.ai.time, 1),
                            "msg": (
                                f"O364:航母硬转化(矿={self.ai.minerals:.0f})"
                            ),
                        })
                    break
        ...
        if (
            ...
            # O369-①a:FB latch 期不产虚空(250 矿/艘正是 latch 要
            # 攒的 300 矿的同台竞争者;latch 解除自动恢复)。
        ...
        # 救的对象不存在,空转比花气更亏。
        # O369-②a(o368a g2 实证):起点口径修复 —— 旧口径「任何在
        # 产即销账」被在产虚空/先知(O261 死窗兜底/oracle 产线)
        # 打断:g2 自救报空转 223s(设计 60s),填线在产 ≠ 舰队
        # 产线复活却把计时反复归零。新口径(sg_idle_reset_needed):
        ...
        _sg_fleet_busy = any(
            (not s.is_idle)
            and s.orders
            and s.orders[0].ability.id
            in (
                AbilityId.STARGATETRAIN_TEMPEST,
                AbilityId.STARGATETRAIN_CARRIER,
            )
            for s in _sg_ready_all
        )
        if sg_idle_reset_needed(len(_sg_ready_all), _sg_fleet_busy):
        ...
        if (
            ...
            for _sg in _sg_ready_all:
                if _sg.is_idle:
                    ...
                    break
        # O369-②b(o368a g3 实证):post-FB 矿穷填线 —— O368-② 自救
        # 按设计在 FB 落成后自灭,但矿穷期 SG 照样空转 153-237s
        # (g3:FB 建成 ~481s→首风暴 590s):舰队产线绑 can_afford
        # (TEMPEST 300 矿),矿 <300 期 SG 全闲零产出。FB 已落、
            ...
            for _sg in _sg_ready_all:
                if _sg.is_idle:
                    ...
                    self.ai._events.append({
                        "t": round(self.ai.time, 1),
                        "msg": (
                            f"O369:post-FB矿穷填线,产虚空"
                            f"(SG空转{self.ai.time - self._sg_idle_since:.0f}s,"
                            f"矿{self.ai.minerals:.0f}<300)"
                        ),
                    })
                    break
        ...
        # O311-③(24→20)已证伪回退(o311b game_01:门放宽后钉点在 456s
        # 波战中拍 Nexus,矿 405 烂银行基地照丢 —— 钉点要在波间隙,
        # 门松让波中拍矿的概率上升)。
        ...
        if (
            ...
            and not (
                self._o364_nexus_fund_hold()
                and nexus_fund_hold_blocks("ROBOTICSFACILITY", 0)
            )
        ):
            _robo_others = [
                th
    ...
    def _wall_build_chain(self, home, force: bool = False) -> bool:
        ...
        # O317-①:普通槽已有 forge(present/pending)则跳过 —— presumed 链
        # 已改为 forge 先行,墙链不再为其另建第二座(150 矿不重复)。
        if (
            UnitID.FORGE not in self._wall_fallback
            and not self._on_wall(_slots, UnitID.FORGE)
            and not self._structure_present_or_pending(UnitID.FORGE)
        ):
    ...
    def _fleet_stall_watchdog(self, structures_dict: dict[UnitID, list[Unit]]) -> None:
        ...
        for sid in watch:
            ...
            # O369-①b(o368a g2 实证):改**累计**口径 —— 不再要求
            # 连续(g2 破产分支 526s 解除后 rush 常亮,连击被打断
            # 再也攒不到 3,FB 到死没落);清零只在 update 解除处
            # (FB 落成/threat/stall 释放)。
            if sid == fb:
                if _retry == "no_money":
    ...
    def _rush_worker_escort(self) -> None:
        ...
        # O364-⑤b(o363a g2 实证):cap 6→3(escort_hard_cap)—— 首波
        # 杀农 15(23→8)其中 6 个是协防拉出塔/电池射程送死的;协防
        # 是拖延(塔下作战/穿矿甩包围)不是决战,3 农塔下足够。
        _keep_mining = max(4, len(self.ai.workers) // 2)
        want = escort_hard_cap(
            enemy_near, len(self.ai.workers),
            keep_mining=_keep_mining,
        )
        ...
        if stance == "tower":
            ...
            # O364-⑤b(o363a g2 实证):作战锚 = 最近的塔或电池(电池
            # 射程=奶程,同教义)—— 协防只在塔/电池射程内作战;旧
            # 「无塔锚 → attack 最近敌」把农民拉出射程送死(g2 协防
            # 6 农全灭实证),锚缺失退回坡口集结点(不接敌)。
            _anchor = min(
                (
    ...
    def _rush_gas_stop(self) -> None:
        ...
        # 条件失效把停气钉成终局状态。
        # O363-④b(o362a/o362b 尸检):解除改纯气压滞回 气<250 ——
        # 「矿 >400 即解」让停气形同虚设(矿一缓就放回气矿,气压不
        # 下去,o362a g3 触发后气照涨 +220);解除只认气压,60s 棘轮
        # 保险丝保留为兜底。
        _gas_pull_cooling = (
        ...
        if (
            ...
            or (
                gas_pull_window_expired(self._o358_gas_pull_since, self.ai.time)
                # O364-③c(o363b g2 实证):棘轮解除加闸 —— 气 >400 且
                # 舰队(TEMPEST+CARRIER)<6 不解除(解除后 +354 漏回
                # 实证:气压没下去、舰队没成型,放回气矿只是再烂一轮);
                # 气压滞回(气<250)解除不受本闸。
                and not gas_stop_release_blocked(self.ai.vespene, _fleet_total)
            )
        ):
    ...
    def _healthy_ready_mining_bases(self, min_slots: int = 15) -> int:
        ...
        python-sc2 会从 mineral_field 移除；矿点本身的
        ideal_harvesters 实测为 0，因此按实时剩余矿点数×2求位数。
        """
        healthy = 0
        for th in self.ai.townhalls.ready:
            slots = mineral_patch_worker_slots(
                self.ai.mineral_field.closer_than(10.0, th.position).amount
            )
            if slots >= min_slots:
                healthy += 1
        return healthy
    ...
    def _want_dynamic_expand(self) -> bool:
        ...
        # 494s 太晚的教训不动);二矿已成交后 400 矿/座的 Nexus 是
        # FB 300/200 的同台竞争者,latch 解除(FB 落成/threat/stall)
        # 自动恢复。
        if self._fb_bankrupt and self.ai.townhalls.amount >= 2:
            return False
    ...
    def _dispatch_structure(
        ...
        if self._o381_nexus_fund_active and sid not in (
            UnitID.NEXUS,
            UnitID.PYLON,
            UnitID.FORGE,
            UnitID.PHOTONCANNON,
        ...
        if sid == UnitID.PHOTONCANNON and not survival_exempt:
            ...
            if cannon_absolute_capped(_cannons_now):
                return "capped"
            if cannon_capped(
                now=self.ai.time,
                fleet_count=(
            ...
            # O364-②:fb_fund_exempt(在建 Nexus 首座塔)豁免本闸 ——
            # o363a g1 塔链被基金窗压到落成后 272s 实证。
            if fb_fund_cannon_blocked(self._fb_fund_window, _cannons_now) and not fb_fund_exempt:
                return "fb_fund"
        ...
        if critical and sid in _PIN_FUSE_FAMILY:
            if (
                ...
                return "pin_fuse_cd"
            # O363-②b(o362b g2 实证):O307 撤派工后的 60s 防夺回封锁 ——
            # 撤派工 4 次每次 30s 冷却一过被同一钉点夺回,二矿拖到 498s;
            # 封锁期内拒同型重钉,资金/科技链真正解锁一轮。
            if pin_repin_blocked(
                self.ai.time, self._o363_repin_block_until.get(sid)
            ):
                return "pin_fuse_cd"
            _pins_now = sum(
                1
        ...
        # O369-③(o368a g1 (162,22) 实证):覆盖清单推广到 needs_power
        # 的防御建筑(PHOTONCANNON/SHIELDBATTERY,2x2 槽)—— g1 首塔
        # O116 not_viable(带电余=0)268-315s 正是被穿窗口,塔钉点
        # 需要同款「先贴槽水晶再钉」;事件 30s 节流(只节流言)。
        if power_precheck_covered(sid.name, needs_power):
            _pc_size = (
                BuildingSize.THREE_BY_THREE
                if sid in (UnitID.STARGATE, UnitID.FLEETBEACON)
                else BuildingSize.TWO_BY_TWO
            )
    ...
    def _dispatch_pin_reanchor(
        ...
        # O369-④(o368a g1 实证):BY/GATEWAY 等 opener 关键链黑名单
        # ≥2 → 不进 O358-④b 的 60s 冷却,直接回退主基内侧常规槽
        # (ares 默认 placement 通道,不带 closest_to)—— g1 BY 换锚
        ...
        if reanchor_fallback_default(
            ...
            if _rc2 == "dispatched":
                self.ai._events.append({
                    "t": round(self.ai.time, 1),
                    "msg": (
                        f"O369:{sid.name}换锚黑{len(st['blacklist'])},"
                        "回退默认槽(不带closest_to)"
                    ),
                })
            return _rc2
    ...
    def _pin_deadlock_sweep(self) -> None:
        ...
        O363-②a:等钱时长改读 _o363_pin_since 跨 pop 累计台账 ——
        O307 撤派工(60s 阈抢先于保险丝)/O110 自救/O118-② 快回收
        都会 pop 重挂条目把 TIME_ORDER_COMMENCED 归零,Nexus/FB 钉点
        单条目口径下永远等不到 60s(o362b g2 钉 290s、o362a g1 FB
        等钱 ~200s 无释放实证)。
        """
        tracker = self.manager_mediator.get_building_tracker_dict
        pins = [
        ...
        # O363-②a(o362b g2/o362a g1 实证):跨 pop 累计等钱台账 ——
        # tracker 条目被 O307 撤派工(60s 抢先于保险丝)/O110 自救清
        # 超龄条目/O118-② 快回收 pop 后重挂,TIME_ORDER_COMMENCED
        # 归零,旧口径的 60s 保险丝永不积累(Nexus 钉点 290s/FB 等钱
        # ~200s 无释放实证)。按 sid 记最早挂出时刻,等钱时长跨 pop
        # 累计;tracker 无该型条目(开工/被撤)即销账,下轮重钉重计。
        _pinned_sids = {_info[TRACKER_ID] for _tag, _info in pins}
        for _sid in list(self._o363_pin_since):
            if _sid not in _pinned_sids:
                self._o363_pin_since.pop(_sid)
        for _tag, _info in pins:
            _sid = _info[TRACKER_ID]
            self._o363_pin_since[_sid] = min(
                self._o363_pin_since.get(_sid, _info[TIME_ORDER_COMMENCED]),
                _info[TIME_ORDER_COMMENCED],
            )
        if not pins:
            return
        ...
        for _tag, _info in pins:
            _sid = _info[TRACKER_ID]
            # O363-②a:等钱时长读累计台账(跨 pop),不再读单条目
            _waiting = self.ai.time - self._o363_pin_since.get(
                _sid, _info[TIME_ORDER_COMMENCED]
            )
            if pin_deadlock_fuse(_workers, _kept + 1, _waiting):
                self.manager_mediator.get_building_counter[_sid] -= 1
                tracker.pop(_tag)
                self._o363_pin_since.pop(_sid, None)
    ...
    def _o364_nexus_fund_hold(self) -> bool:
        """O364-①(o363a g3 直接死因):Nexus 资金窗独占期判据 ——
        纯函数 nexus_fund_hold_active 的实例包装。首扩等钱 >60s
    ...
    def _free_3x3_slots_at(self, base_location, size=None) -> list:
        """O113-①:某基地 3x3 槽 [(x, y, free), ...] —— 贴槽落水晶的锚点源。
        O369-③:size 参数(默认 THREE_BY_THREE 保持旧口径)—— 塔/电池
        钉点的供电预检要贴 2x2 空闲槽落水晶。"""
        if size is None:
        ...
        return [
            ...
            for pos, v in placements[bloc].get(size, {}).items()
        ]
    ...
    async def _build_flow_structures(
        ...
        if (
            ...
            # O363-③(o362a 尸检):t≥240 后连环门豁免 —— SG 钉点 401s 仍
            # 等 Nexus 同 tick 才放行(g3),SG 354-450s 全超 ≤300s 线;
            # t≥240(cyber 就绪在上游)后 SG 与扩张并行预算。
        ...
        if not core_allowed:
            ...
            # O364-①:Nexus 资金窗独占期(O336 等钱 >60s 武装)FB 也
            # hold —— o363a g3 实证 FB 325s 的 300/200 正是挤掉 Nexus
            # 400 矿窗的科技消费之一;成交/45s 超时/threat 放行。
            if self._is_zerg_timing_fb_exempt() and not (
                self._o364_nexus_fund_hold()
                and nexus_fund_hold_blocks("FLEETBEACON", 0)
            ):
                if (
            ...
            if structure_id == UnitID.FLEETBEACON:
                ...
                if (
                    ...
                    if self._opp_race == "zerg" and self._ai_build == "timing":
                        ...
                        # O364-①:Nexus 资金窗独占期 FB 钉点 hold
                        if (
                            _voids_now >= 1 or self.ai.time >= 320.0
                        ) and not (
                            self._o364_nexus_fund_hold()
                            and nexus_fund_hold_blocks("FLEETBEACON", 0)
                        ):
