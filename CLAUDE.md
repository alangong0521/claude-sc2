...
## 流派（BUILD env，开局前锁定）
`BUILD=tempest|stalker`，**启动参数**，不能局中换。
- `run.py:22` 在 `import bot.main` **之前** `os.environ.setdefault("BUILD", ...)`——因为 `production_manager` 在**模块加载时**就读 `BUILD`（`BUILD_FLOW`/`is_stalker_flow` 模块级常量）。
- 切换：`BUILD=stalker poetry run python run.py`（在 `ares-bot/` 下）。

## 关键架构（改动热点）
- **`bot/managers/production_manager.py`** — Protoss macro 主战场。模块级常量按流派分支（`CORE_STRUCTURES`/`DESIRED_UPGRADES`/`_STALKER_SPAWN`/`EXTRA_CORE_STRUCTURES`）。`update()` 每帧注册 macro behaviors。**改产能/升级/兵种/防御大多在这**。
...
## 约束 / 踩过的坑
- **`army_composition.yml` 被 `tests/test_army_config.py::test_shipped_protoss_upgrades_unchanged` 锁**——改升级走 `DESIRED_UPGRADES`，**不碰 yaml**。
- **`spawn_controller.py` 比例和必须 = 1.0**——混编比例注意。
...
# 测试
PYTHONPATH=ares-sc2/src:ares-sc2/src/ares:ares-sc2 poetry run pytest tests/ -q   # 74 passed
# 跑局
