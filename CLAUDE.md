...
- **bot 局小地图点击"失灵"**（2026-07-19 结案）：三层叠加——①窗口未成为 key window 时点击被当"激活"吞掉（先点主画面）；②AI 投降弹窗是模态框挡全部输入（有 gg 聊天型(bench 自动点 Yes)和静默型(手动点)）；③**主因:REALTIME=False 全速模拟下 UI 线程被饿死,离散点击被间歇性丢弃(合成/物理点击都会丢,时好时坏)——解法:按住小地图**拖动**代替单击(连续事件流丢不掉),观察优先用 --realtime 局**。判别手法：手动开一局 vs AI 能点 = bot 局特有。
