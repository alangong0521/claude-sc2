...
> **2026-08-16 司令确认（O288 起）**：每轮验证 **5 局**即可，不再默认 10 局；**同日司令再确认（O289 起）：双 lane 并行（各 5 局）提高验证效率**，headless `REALTIME=False` 不变。
>
> **2026-08-18 司令拍板（O342 起）**：验证协议改为 **双 lane 同组合 ×3 局
> （合计 6 局/轮）**——主攻组合（当前 VeryHard Zerg Timing）两条 lane 都
> 跑它，每轮 6 局统计优于单 lane 5 局且速度快 ~40%；已打穿组合（Harder
> Zerg Timing,o340b 5/5）改为**每 3 轮插一次回归 lane**（lane2 换成
> Harder×3）。3 局制噪声大（单局 ±33%），所有判读强制**两轮累计**才下
> 结论（单轮 6 局的 2/6→0/6 不算回归）。
...
> **7200s**（1800s 会在游戏约 1000s 误杀；3600s 仍会在游戏约1540s、
> 舰队满编且时间持续推进的正常长局误杀）。
>
...
> ①**农民效率**——采矿饱和/分矿均衡（主基 16+16 满载 vs 分矿 2-3 农）、
> idle_builder 干等、气矿 1-2 农 vs 钱矿爆仓的再分配；②**战斗部队分配**——
> 敌主力来袭时是否全员参战（而非只派 2 个空军）、战后残留小股敌人是否
> 被清剿、前线骚扰与主场防守的兵力切分；③**科技树节奏**——核心链
> （cyber/SG/FB）落成时点 vs 资金去向（气烂银行 = 链断信号）、升级研究
> 是否被让位闸长期冻结；④**开矿时机与经济差（司令 2026-08-17 二补）**——
> **二矿晚于 5 分钟（300s）落地的局全部重点分析**；神族应保证 ≥2 个健康
> 矿场常态工作、3 矿区同采是正常状态。**健康矿区按就绪 Nexus 周围剩余
> 采矿位 ≥15 计算；45+ 农时目标实时维持 3 片，否则至少 2 片。在建基地
> 不能提前冒充健康矿区。**任一分矿被端掉后，优先攒 400 矿恢复 Nexus，
> 暂停新增作战单位、升级、科技与扩产（只留生存级供给/必要防御和农民硬
> 底线），禁止幸存农民跨矿区低效采集时继续烧钱。carrier 流严重依赖气矿，多数败局
> 终因是经济差距滚雪球：每轮尸检必须对比敌我经济曲线（农民数/基地数/
> 分钟收入），定位经济差拉开的时间点和原因（开矿晚？农民断训？基地被拆
> 重建慢？），改进点优先补经济短板。
> 每个维度用快照数据说话（农民曲线/气矿存款/
> 终局编成/科技落成时点），不接受无数据支撑的「感觉」。
>
> **录像对照分析（司令 2026-08-18 三补）**：每局对局结束，除日志尸检外，
> 必须解析对局录像（bench 加 `--replay` 后每局自动存
> `bench/<tag>/game_XX/replay_XX.SC2Replay`（正式 bench 默认带 `--replay`），
> 解析工具 `scripts/replay_bases.py`，可扩展），**对比电脑与我方的建造顺序
> 差距与经济曲线**（基地落成时间线/农民数/分钟收入），从中找到至少 3 个
> 优化点落地迭代。重点是从录像找我方差距。参照基准（2026-08-18 实测）：
> 电脑 VeryHard Zerg **二矿 119-150s、三矿 ~660s**；我方二矿开动目标
> **120s 向电脑看齐**，二矿晚于 300s 落地的局全部重点分析（原 5 分钟线
> 并轨收紧）。
>
> **防御部署原则（司令 2026-08-18）**：防御塔+电池**聚在一起**才有效。
> 放弃主基地分散铺塔，优先速开 2 矿，防御重心整体部署在 **2 矿主基地
> 附近**（足够多的塔+电池+堵口建筑学）；新矿未站稳前不在其附近零散
> 铺塔（3 矿边 1-2 座零散塔电池 = 敌一波推上高地的白捐，司令观战实证）。
>
> **O382 经济/进攻纪律（司令 2026-08-20）**：首扩基金从 220s 启动并截断仍在
> 消费的 opener，目标 Nexus 实体约 240-270s；健康矿区必须按「Nexus 10 格内
> 实时剩余矿点数×2」计算，不能读矿点 `ideal_harvesters`（实测为 0）。45+ 农
> 实时维持 3 片健康矿区，不足时四/五矿扩张不允许被 O307 超时撤派。Terran
> 可见空中作战单位与硬对空清零、我方舰队 ≥8 后，已通过安全闸的舰队优先打
> 最外围已知分矿，斩断经济；全局塔绝对上限 18，只有新矿零塔的首座生存塔豁免。
> **O383 Terran Rush 适配**：本组实测首接触常在 500s+，因此 420s/
> 首接触前最多 2 塔+2 地面保底，钱优先 Nexus→BY→SG→FB。丢矿基金
> 不允许机械重拍原死亡点；有可见敌地面主力时选距其最远的安全矿点。
> Nexus 恢复后 120s 只留新矿首塔，第 2+塔/电池、升级和额外产能让位
> 核心舰队产线，避免「47 农+1000气但 0 SG/0 舰队」。
> **O383 Zerg 胜局精修**：健康矿区触发扩张后必须 latch 到 Nexus
> 实体数+1，不能因 worker/tracker 让 nexus_pending 翻真就被 O307 撤派。
> 新矿首塔基金窗对齐90s验收（原45s提前释放导致首塔128s才成交）。
> 首艘暴风/航母必须真实出场后才允许第3+星门，在产队列不得冒充
> 战场产出抢首舰资金窗。
> 首塔基金只解决“缺150矿”问题：若窗口≥15s时已经买得起塔、且无塔工台账/
> 在途，说明失败在 placement/no_worker/tech，不得继续冻结全局90s；释放基金，
> 15s后走落点/派工重试。
> **O385 Zerg 胜局精修**：首塔非资金失败除释放基金外必须累计该基地锚点失败，
> 3次后直接进入水晶环带 fallback，不能15s后无限重试同一扇形。分矿防御自救
> 局部水晶最多3根，避免“0塔却5水晶”。四矿且健康矿区已达2-3片目标时停止
> 常态饱和五矿；仅健康矿区跌破目标时再开五/六矿。
> **O384 Terran 后半局纪律**：预接触塔帽必须落在最终 F2/PSD 目标层，
> 手动派工入口的 cap 不能代表全协议覆盖；420s/首接触前主基与每个分矿各最多
> 1塔/1电池。制空斩分矿属于可撤经济打击，任一基地出现10个地面威胁即召回，
> 不沿用普通决死推进的14/25门。丢矿安全点必须距主基≤80格并带0.5回防距离权重，
> 禁止为了“离敌最远”拍到无法供电/协防的地图远角。
