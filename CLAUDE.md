...
- **基地清零重建（O15）**：`townhalls==0` 时 MacroPlan 只留 AutoSupply（研究/出兵/开销块全停，攒钱 400 重建，优先级 > save_up）；主矿干则 `ExpansionController(to_count=1)` 找新矿；Q5 判负豁免条件=有工人且场上还有矿（`nexus_rebuild_viable`）。
